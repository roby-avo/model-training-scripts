...
def train_model(training_file, columns_to_exclude, target_column, json_config_file, output_dir):
    ...
    model_filename = os.path.join(output_dir, f"model_{os.path.basename(training_file).split('.')[0]}.h5")
...
def main():
    ...
    parser.add_argument('training_files_dir', type=str, help='Directory containing the training CSV files.')
    parser.add_argument('output_dir', type=str, help='Directory to save the output models and reports.')
    ...
    training_files = [os.path.join(args.training_files_dir, f) for f in os.listdir(args.training_files_dir) if f.endswith('.csv')]
    
    for training_file in training_files:
        print(f"Training model on {training_file}...")
        train_model(training_file, args.columns_to_exclude, args.target_column, args.json_config_file, args.output_dir)
